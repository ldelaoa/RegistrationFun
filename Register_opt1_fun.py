...
def Register_fun(planning_ct_np, lowdose_ct_np, pet_np, patient_number):
    ...
    #Initial point is center of both images
    ...
    registration_method.SetOptimizerAsGradientDescent(learningRate=1.0, numberOfIterations=400,
    ...
    print(
        f"Final metric value for patient {patient_number}: {registration_method.GetMetricValue()}")  # MattesSimilarityMetric is similar to NCC, both use correlation in intensity
    print(
        f"Optimizer's stopping condition for patient {patient_number}: {registration_method.GetOptimizerStopConditionDescription()}")
    ...
    registered_pet_image = sitk.Resample(pet_image, fixed_image, final_transform, sitk.sitkLinear, 0.0,
                                         moving_image.GetPixelID())
    if False:
